...
#Clean DataStore(s)
	for d in $(cat deploy-photon/manifests/photon.yml | grep DATASTORE | awk -F ":" '{print $2}' |  tr "," "\n" | sort -u); do
		...
		vifs --server $HOST --username $ESX_USER --password $ESX_PASSWD --rm  "[$d] photon-installer" --force || echo "Already Wiped"
	        vifs --server $HOST --username $ESX_USER --password $ESX_PASSWD --rm  "[$d] photon-cli" --force || echo "Already Wiped"
